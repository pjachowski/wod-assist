...
- **Supabase migrations**: `supabase/migrations/<YYYYMMDDHHmmss>_<short_description>.sql`. Always enable RLS and write per-operation, per-role policies.
- **React hooks**: extract to `src/components/hooks/`. Services / business logic to `src/lib/` (or `src/lib/services/`).
